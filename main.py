...
def main():
    ...
    while running:
        ...
        if inventory_popup.visible:
            ...
            inventory_popup.draw_flower_images(screen)

        # Check for mouse click
        if pygame.mouse.get_pressed()[0]:  # Left mouse button
            mouse_x, mouse_y = pygame.mouse.get_pos()
            for flower in orchard.flowers:
                if flower.get_rect().collidepoint(mouse_x, mouse_y):
                    if flower.visible:  # Only process if the flower is visible
                        if flower.sprite_rect == blue_flower_rect:
                            player.add_item_to_inventory("Silent Petal")
                        elif flower.sprite_rect == pink_flower_rect:
        ...
        text = f"Silent Petal: {player.inventory.get_items().get('Silent Petal', 0)}, Fire Blush: {player.inventory.get_items().get('Fire Blush', 0)}"
